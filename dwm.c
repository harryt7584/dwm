...
drawbar(Monitor *m)
{
	...
	if (m == selmon) { /* status is only drawn on selected monitor */
		tw = m->ww - drawstatusbar(m, bh, stext) - tw - 2 * sp;
	}
	...
	for (i = 0; i < LENGTH(tags); i++) {
		...
		if (occ & 1 << i) {
			...
			drw_rect(drw, x + boxw + 2, 0, w - ( 2 * boxw + 1) - 4, boxw - 2,
...
setup(void)
{
	...
	scheme[LENGTH(colors)] = drw_scm_create(drw, colors[0], alphas[SchemeNorm] ,3);
